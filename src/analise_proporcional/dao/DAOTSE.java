...
import analise_proporcional.negocio.VotacaoSecao;
import analise_proporcional.negocio.BemDeCandidato;
import analise_proporcional.negocio.VotacaoPartido;
...
public class DAOTSE extends AbstractElectionDAO {
    ...
    public DAOTSE() throws ClassNotFoundException, InstantiationException, IllegalAccessException {
        ...
        userName = "ronny";
        //
        this.NOME_COD_CARGO = new TreeMap<String, String>();
        this.NOME_COD_CARGO.put("\"SENADOR\"", "\"5\"");
        this.NOME_COD_CARGO.put("\"DEPUTADO FEDERAL\"", "\"6\"");
        this.NOME_COD_CARGO.put("\"DEPUTADO ESTADUAL\"", "\"7\"");
        ...
        try {
            ...
            for (Object ob : ftr.getRows()) {
                ...
                query = String.format("insert into Candidatura values ('%s',"
                        ...
                        cand.getDescricaoCargo(), cand.getSeqCandidato());
                stmt.execute(query);
                //stmt.close();
            }
            stmt.close();
        ...
        try {
            ...
            for (Object ob : ftr.getRows()) {
                ...
                query = String.format("insert into VotacaoCandidato values ('%s',"
                        ...
                        vc.getNumTurno(), vc.getSiglaUF(), vc.getSeqCandidato());
                stmt.execute(query);
                // stmt.close();
            }
            stmt.close();
    ...
    public ArrayList<Candidatura> getPerfisCandidaturas(String regiao, String cargo, final boolean ordByGastos) throws SQLException {
        ...
        query =   " SELECT * FROM Candidatura"
                ...
                // + " USE INDEX ( Candidatura.SeqCandidato, VotacaoCandidato.SeqCandidato )"
                + " ON "
    ...
    public ArrayList<Candidatura> ordenaCandidaturasPorDominancia(ArrayList<String[]> entries){
        // Se nao eleito, set totalVotos pra 0
        // Se eleito, set totalVotos para a soma da legenda
        //
        // Calcula a soma dos votos de cada legenda
        TreeMap<String, Candidatura> dominancias = new TreeMap<String, Candidatura>();
        TreeMap<String, Integer> domLegendas = new TreeMap<String, Integer>();
        ArrayList<Candidatura> perfis = new ArrayList<Candidatura>();
        for (String[] entry : entries) {
            Candidatura cand = new Candidatura();
            cand.setAll(entry);
            String seq = cand.getCodigoLegenda();
            int votes = Integer.parseInt(entry[43].replaceAll("[\\D]", ""));
            if(domLegendas.containsKey(seq)){
                int total = domLegendas.get(seq);
                domLegendas.replace(seq, total + votes);
            } else {
                domLegendas.put(seq, votes);
            }
        }
        //
        for(String[] entry: entries){
            ...
            String seq = cand.getSeqCandidato();
            if(!dominancias.containsKey(seq)){
                if(entry[37].compareTo("\"ELEITO POR QP\"") != 0
                   && entry[37].compareTo("\"ELEITO POR MÉDIA\"") != 0){
                    cand.setTotalVotos(0);
                } else {
                    cand.setTotalVotos(domLegendas.get(cand.getCodigoLegenda()));
                }
                dominancias.put(seq, cand);
            }
        }
